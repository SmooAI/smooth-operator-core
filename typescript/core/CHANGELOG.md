...
- 12d3787: fix(all): make memory auto-recall byte-identical across the five cores (th-ffaeae)
  ...
  - **The verify-before-recommend note now reaches every engine.** Only Rust told
    ...
    _past_; on the other four it happily recommended symbols deleted three releases
...
- d8ab559: fix(release): actually publish the TS/Python/.NET temporal packages (th-8a0b45)
...
- 766a488: feat(temporal): the Rust client-side `TemporalExecutor` — the reference language catches up with its four ports (ADR-030, th-db0816)
...
- 5c151b1: fix(go): a truncated stream is an error, not a clean finish; SEP hooks run on RunStream; no goroutine/socket leak
...
- de9edcc: go/typescript: split streamed mock text on **character** boundaries, not bytes / UTF-16 code units.
...
- 34ac8c9: Python: close a fail-open hole in `Clearance` and bound the human-in-the-loop gate.

  **`Clearance` had no `"*"` wildcard, so a deny-all clearance failed OPEN.** Rust's
  `Clearance::deny_all()` _is_ `deny_tools = ["*"]`, and its `allows()` matches
...
- 6d94207: Fix a `Clearance` fail-open on a `"*"` deny entry in the TypeScript, Go and .NET engines.
  ...
  _is_ `deny_tools = ["*"]`. The other three engines compared deny entries **literally**
  ...
  that Rust has no equivalent of, and _that_ path always worked. So the two ways of
  ...
  Verified per language by reverting _only_ the semantic line — leaving the new
...
- 6be105a: Fix a permission bypass on the .NET durable execution path, and stop it dropping thread history.
  ...
  **The dropped thread.** `TemporalExecutor.BuildInput` accepted a `SmoothAgentThread` and never read
  ...
  and a tool result stripped of its call id, which the _next_ turn's model rejects.
