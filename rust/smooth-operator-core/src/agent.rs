    ...
    fn spawn_preamble(&self, user_msg: &str, tx: &tokio::sync::mpsc::UnboundedSender<AgentEvent>) {
        ...
        let provider: Arc<dyn LlmProvider> = match &self.llm_provider {
            ...
            None => {
                ...
                // Same spend attribution as the main model — the preamble is a
                // billed call, so it must not land unattributed in the gateway's
                // spend logs.
                Arc::new(LlmClient::new(fast_cfg).with_metadata(self.config.request_metadata.clone()))
            }
        };
