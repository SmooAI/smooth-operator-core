...
Every engine supports the same core capabilities below. Beyond this shared core, the Rust reference carries surfaces still being ported (multimodal images and the extension sandbox/integrity hardening) — if a capability matters to you in a non-Rust engine, check that language's package docs before assuming it. A real gateway LLM client is no longer one of them, and neither is provider routing: all five now ship both. **Anthropic `cache_control` request markers** now ship in all five: the marker goes on the last system message, the last tool, and the last history message, gated on a Claude-routing model id plus a LiteLLM-style or `anthropic.*` base URL.
