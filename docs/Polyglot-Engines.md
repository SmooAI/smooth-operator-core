...
Every engine supports the same core capabilities below. Beyond this shared core, **one** surface is still Rust-first: the extension **sandbox / integrity hardening** (see [Extension-Sandboxing-Design.md](Extension-Sandboxing-Design.md)) — capability *declarations* are honoured in all five, but the process-level confinement and manifest-integrity verification are Rust-only today. Multimodal images, structured output, a real gateway LLM client and provider routing are no longer on that list: all four now ship in all five. For the launch-facing summary — what is at parity, what is server-side only, and what is genuinely left — see [PARITY-STATUS.md](https://github.com/SmooAI/smooth-operator/blob/main/PARITY-STATUS.md). **Anthropic `cache_control` request markers** now ship in all five: the marker goes on the last system message, the last tool, and the last history message, gated on a Claude-routing model id plus a LiteLLM-style or `anthropic.*` base URL.
...
- **Multimodal image input** — attach images to a user turn as a `data:` or `https` URL, emitted as an OpenAI `image_url` content part with an optional detail hint. In all five; Go carries it on `ImageContent` in the agent rather than a separate module.
- **Tool-hook lifecycle** — `pre_call` runs before every tool (raise/throw/return-error to BLOCK it) and `post_call` runs after with a **mutable** result, which is the redaction seam the NarcHook above plugs into. In all five, including the mutable-result contract.
- **Permission gate + deny-policy + grants** — a deterministic allow/ask/deny classifier over every tool call (`AutoMode`: ask / accept-edits / deny-unmatched / bypass), hard circuit-breakers that no mode downgrades, a consumer-supplied `DenyPolicy` evaluated first, and a stored grant allow-list that can only upgrade an `Ask` — never waive a `Deny`. In all five, exhaustively tested against adversarial compound-command and credential-path inputs.
- **SEP extension host** — spawn extension subprocesses over the Smooth Extension Protocol: manifest parsing, capability negotiation, the tool/provider/UI/command lanes, restart backoff, and a bounded oldest-shedding observe lane that emits an `events_lost` marker on recovery. In all five, held to the shared conformance fixtures.
- **Gateway cost headers** — read the per-request USD cost the gateway reports in a response header and fold it into the `CostTracker`, on the streaming path too (where the parsed response object is gone by the time the stream ends). In all five.
- **Provider routing** — a registry of provider credentials/URLs plus a per-activity routing table, so each `Activity` (coding, reasoning, reviewing, judge, summarize, fast) picks its own model. Five presets, nine provider factories, per-slot fallback chains, per-model wire quirks, and LiteLLM alias resolution (`GET /model/info`). The registry reads and writes the same `~/.smooth/providers.json` in every engine, and the routing values are pinned across all five by the shared corpus at [`spec/providers/routing.json`](../spec/providers/routing.json).
...
- **Durable-execution seam (ADR-030)** — an `AgentExecutor` that decides *where* a turn runs, over an `AgentActivities` + `driveTurn` split that separates the side-effecting model/tool calls from the deterministic loop sequencing them. The default in-process executor is a verbatim delegation to that engine's own `run`, so the seam changes nothing until something plugs into it. All five engines now ship an actual durable **backend**: a Temporal-backed executor that runs the turn as a workflow with the model and tool calls as activities, giving crash-safe resume, durable human-in-the-loop via approve/deny signals, and a durable-wait timer. Each mirrors the Rust crate and ships as a **separate, optional per-language package** — Rust's feature-gated `smooth-operator-temporal` crate, the Go `go/temporal` module, TS `@smooai/smooth-operator-temporal`, Python `smooai-smooth-operator-temporal`, and .NET `SmooAI.SmoothOperator.Temporal` — each verified by a skip-gated e2e against a real ephemeral Temporal server. The Temporal SDK lives in that optional package, not the engine, so no engine pulls a Temporal SDK into your dependency tree. Shared ADR-030 follow-ups remain open in all five (including Rust): the durable path yields only a terminal result (no token-delta streaming) and reports `costUsd = 0` on the workflow result, and the executor seeds from agent config only — the workflow→streaming adapter bridge that closes these is still to come.
...
- **Keep hold of the response on the streaming path.** Per-request cost is reported ONLY in a response header. Headers are not destroyed by reading the body — a fetch `Response`, Go's `http.Response` and .NET's `HttpResponseMessage` all stay readable afterward. The real hazard is narrower: a streaming call hands back **only a stream** unless you explicitly ask for the response (`.withResponse()` / `with_raw_response` / `ResponseHeadersRead`; in Rust `bytes_stream()` takes `self` by value, so the response is *moved away*). Drop it and the cost is not late, it is absent. Each client captures the response and carries the cost forward (on the response when blocking; on a leading chunk when streaming), so it folds into the turn's `costUsd` even if the stream errors before usage arrives. This is the regression fixed in Rust by core#102 and swept across the ports by core#121 — when reviewing it, ask "is the response still captured?", not "is this read early enough?". The candidate list and its first-**non-zero** precedence is shared: `x-litellm-response-cost-margin-amount` → `-original` → `x-litellm-response-cost` → `x-response-cost` → `x-cost-usd`. A header that is present and reports `0` is NOT a recorded $0 — it falls through, and if nothing measures, the local pricing estimate is used instead.
